...
from PERT.PinyinCharDataProcesser import PinyinCharDataProcesser
from PERT.NEZHA.modeling_nezha import BertConfig, BertForTokenClassification
from PERT.dssm_process import threshold, pinyin_file, chinese_file
...
class Py2WordPERT(object):
    ...
    def ConvertPinyinListToCharList(self, pinyinList):
        # print("pinyinList: ", len(pinyinList))
            ...
            with open(rsltCharFile, 'w', encoding='utf-8') as fOut:
                for line in tqdm(fIn.readlines()):
                    ...
                    try:
                        ...
                        print("predition: ", outline)
                        fOut.write(outline)
                    except:
                        fOut.write('error\n')
                        # 抓出出錯的 data 另外寫成 .txt 然後程式繼續往下跑
                        with open(exceptionFile, 'a+') as excep:
                            excep.write(str(line.strip()))
                            excep.write("\n")
                            # excep.write(str(outline))
                            # excep.write("\n")
                        print('Error!!!\n')
                        continue
        pass
...
def EvaluateOnFiles(goldenCharFileIn, rsltCharFileIn, logOut): #logFile => logOut
    totalCount = 0
    correctCount = 0
    with open(logOut, 'w', encoding='utf-8') as fOut: #logOut
...
parser.add_argument('--charLex', type=str, default="./PERT/Corpus/CharListFrmC4P.txt",
                    help='location of the char lexicon file')
parser.add_argument('--pyLex', type=str, default="./PERT/Corpus/pinyinList.txt",
                    help='location of the pinyin list')
parser.add_argument('--pinyin2PhrasePath', type=str, default="./PERT/Corpus/ModernChineseLexicon4PinyinMapping.txt",
...
parser.add_argument('--bigramModelPath', type=str, default="./PERT/Models/Bigram/Bigram_CharListFrmC4P.json",
                    help='location of char bigram file')
parser.add_argument('--modelPath', type=str, default="./PERT/Models/pert_tiny_py_lr5e4_10Bs_1e/",
                    help='this is the path to bert model which contains the model file and the config file')
parser.add_argument('--charFile', type=str, default=chinese_file,
                    help='location of the char corpus')
parser.add_argument('--pinyinFile', type=str, default=pinyin_file,
                    help='location of the pinyin corpus')
parser.add_argument('--conversionRsltFile', type=str, default="./PERT/Logs/Eval_ConvertRslt_DWAVE_test_0{threshold}.txt".format(threshold=str(threshold)[2:]),
                    help='this is the file to contain the conversion result')
parser.add_argument('--logFile', type=str, default="./PERT/Logs/Eval_Rslt_DWAVE_test_0{threshold}.txt".format(threshold=str(threshold)[2:]),
...
pinyinList = ['wo', 'zhi', 'xiang', 'zhi', 'dao', 'ni', 'shi', 'fou', 'zhen', 'de', 'ai', 'wo']
charList = thePy2WordPERT.ConvertPinyinListToCharList(pinyinList)
print(pinyinList)
print(charList)
# print("len(charList)", len(charList))

exceptionFile = "./result/exceptionSongTitle.txt"
...
try:
    ...
    assert len(pinyinList) != len(charList), "Three possible conditions as follow: \n 1. Length of pinyin and Chinese are not match.\n 2. It's English not pinyin.\n 3. The data contains feat info."
